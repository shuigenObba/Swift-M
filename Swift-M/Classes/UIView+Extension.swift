...
public extension UIView {
    public func round(byRoundingCorners: UIRectCorner = UIRectCorner.allCorners, cornerRadi: CGFloat) {
        self.roundSize(byRoundingCorners: byRoundingCorners, cornerRadi: CGSize.init(width: cornerRadi, height: cornerRadi))
    }
    
    public func roundSize(byRoundingCorners: UIRectCorner = UIRectCorner.allCorners, cornerRadi: CGSize) {
        guard let maskLater = self.layer.mask else {
            let rect = self.bounds
            let bezierPath = UIBezierPath(roundedRect: rect, byRoundingCorners: byRoundingCorners, cornerRadii: cornerRadi)
            defer {
                bezierPath.close()
            }
